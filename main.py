...
# --- ÇEVRESEL DEĞİŞKENLERİ YÜKLE (GitHub Güvenliği İçin) ---
load_dotenv()

# --- LOGLAMA YAPILANDIRMASI ---
logging.basicConfig(format="%(asctime)s - %(name)s - %(levelname)s - %(message)s", level=logging.INFO)
logger = logging.getLogger("QuantBot_v3_Prod")
...
# --- 1. CANLI VERİ VE BORSA YÖNETİMİ ---
class ExchangeManager:
    def __init__(self):
        # .env dosyasından okunan güvenli API bilgileri
        api_key = os.getenv("BINANCE_API_KEY", "")
        secret = os.getenv("BINANCE_SECRET", "")
        self.exchange = ccxt.binance({'apiKey': api_key, 'secret': secret, 'enableRateLimit': True})
        
    def fetch_live_data(self, symbol: str, timeframe: str = '15m', limit: int = 300) -> pd.DataFrame:
        try:
            if "USDT" in symbol or "BTC" in symbol:
                bars = self.exchange.fetch_ohlcv(symbol.replace("/", ""), timeframe, limit=limit)
...
class DatabaseManager:
    def __init__(self, db_file: str = "trading_bot_prod.db"):
    ...
    def _init_db(self):
        with self._get_connection() as conn:
            ...
            cursor.executescript("""
                ...
                INSERT OR IGNORE INTO portfolio (id, balance) VALUES (1, 1000.0);
            """)
            default_assets = [("Kripto", "BTC/USDT"), ("Kripto", "ETH/USDT"), ("BIST", "THYAO.IS"), ("NASDAQ", "AAPL")]
...
class TechnicalAnalyzer:
    ...
    def custom_pin_editor_logic(cls, df_15m: pd.DataFrame, df_4h: pd.DataFrame, ml_opt: MLOptimizer, open_trade: tuple = None) -> Optional[TradeSignal]:
        if df_15m.empty or len(df_15m) < 30 or df_4h.empty or len(df_4h) < 50:
            return None

        # ML Destekli Dinamik Eşikler
        rsi_oversold, rsi_overbought = ml_opt.get_dynamic_rsi_thresholds()

        # 4H EMA Trend Kontrolü (Sizin Pine kodunuzdaki Makro Filtre)
        ...
        # SİZİN PINE SCRIPT MANTIĞINIZ (BOS / CHoCH: Son 5 barın tepesini kırma)
        ...
        # Sinyal Koşulları (Pine Script'teki "longCondition" ve "shortCondition" birebir uyarlandı)
...
class ChartGenerator:
    ...
    def generate_chart(df: pd.DataFrame, symbol: str, signal: Optional[TradeSignal]) -> str:
        ...
        ax1.plot(df.index[-100:], df['close'].iloc[-100:], label='Fiyat', color='#2962FF', linewidth=1.5)
        ax1.plot(df.index[-100:], df['high'].rolling(5).max().iloc[-100:], color='#00E676', linestyle='--', alpha=0.5)
        ...
        ax1.set_title(f"{symbol} Analizi (ML + Piramitleme Entegreli)", fontsize=14, fontweight='bold')
        ax1.legend(loc='upper left')
        ax1.grid(True, alpha=0.3)

        rsi = TechnicalAnalyzer.calculate_rsi(df['close'])
        ax2.plot(df.index[-100:], rsi.iloc[-100:], color='#AA00FF', linewidth=1.5)
...
class TradingBotApp:
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        keyboard = [
            [InlineKeyboardButton("🪙 Kripto (CCXT)", callback_data="cat_Kripto"), InlineKeyboardButton("📈 BIST/Forex (YF)", callback_data="cat_BIST")],
            [InlineKeyboardButton("💼 Sanal Portföy", callback_data="portfolio"), InlineKeyboardButton("🧠 ML Modeli Durumu", callback_data="ml_status")]
        ]
        await update.message.reply_text("🤖 **V3 Kantitatif Sistem Aktif**\nCanlı Veri, Piramitleme ve Yapay Zeka devrede.", reply_markup=InlineKeyboardMarkup(keyboard), parse_mode="Markdown")
    ...
    async def button_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data.startswith("cat_"):
            ...
            await query.edit_message_text(f"📂 **{cat}** Listesi (Canlı Veri):", reply_markup=InlineKeyboardMarkup(kb), parse_mode="Markdown")
            
        elif data == "ml_status":
            r_down, r_up = self.ml_optimizer.get_dynamic_rsi_thresholds()
            msg = f"🧠 **Hata Öğrenme Döngüsü (ML Optimizer)**\n\nGeçmiş işlemler analiz edilerek sınır değerleri dinamik olarak güncellendi.\n\n🔻 Optimize Oversold (Alım): `{r_down:.2f}`\n🔺 Optimize Overbought (Satım): `{r_up:.2f}`"
            ...
            await query.edit_message_text(f"🔍 `{symbol}` canlı verilerle ML eşliğinde analiz ediliyor...", parse_mode="Markdown")
            
            df_15m = await asyncio.to_thread(self.exchange.fetch_live_data, symbol, "15m", 300)
            df_4h = await asyncio.to_thread(self.exchange.fetch_live_data, symbol, "1h", 300)

            if df_15m.empty:
                await query.edit_message_text("⚠️ Veri çekilemedi. Borsa API'sini veya sembolü kontrol edin.")
                return
            ...
            caption = f"📊 **{symbol} Canlı Analiz Raporu**\n\n"
            if signal:
                ...
                caption += "⏳ Trend veya ML algoritması yapı kırılımı teyit edemedi. Piyasa izleniyor."
    ...
    def run(self):
        logger.info("QuantBot_v3_Prod Başlatılıyor...")
        self.app.run_polling()

if __name__ == "__main__":
    # .env dosyasından token okunuyor. Koda açıkça token YAZILMAMALIDIR!
    TOKEN = os.getenv("TELEGRAM_BOT_TOKEN")
    
    if not TOKEN:
        logger.error("HATA: .env dosyasında TELEGRAM_BOT_TOKEN bulunamadı!")
        print("Lütfen .env dosyanızı oluşturun ve TELEGRAM_BOT_TOKEN değişkenini tanımlayın.")
    else:
